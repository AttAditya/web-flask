...
function display_data(data, tagname, reset=true) {
	if (reset) {
	...
	document.getElementById("tagged").innerHTML += `
		<div class="has-text-centered column is-full is-hidden">
			<a href="/tag/${tagname}" target="_blank" rel="noopener noreferrer">
...
async function load(catname) {
	await fetch(`/category/${catname}/get`).then(req => req.json()).then((tags_list) => {
		document.getElementById("tags").innerHTML = ``;
		for (let tag of tags_list) {
			document.getElementById("tags").innerHTML += `
				<button class="m-2 button is-danger is-medium" onclick="view_tag('${tag.name}');">
			...
			// await view_tag(tag.name, false);
		}

		if (!tags_list.length) {
			document.getElementById("tags").innerHTML = `
				(No articles/tags under this category for now)
			`;
		}
	});
}

async function view_tag(tagname, reset=true) {
	let topics = {}
	topics[tagname] = 10

	document.getElementById("load-button").classList.remove("is-hidden");
	document.getElementById("tagged").classList.add("is-hidden");
	...
	await fetch(`/articles/meta`, {
		...
		display_data(res, tagname, reset);
		
		document.getElementById("load-button").classList.add("is-hidden");
		document.getElementById("tagged").classList.remove("is-hidden");
	}).catch(_error_ => {
